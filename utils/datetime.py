...
def get_past_datetime(x, iso_format=False, now=datetime.datetime.now()):
    if not isinstance(x, int):
        return ValueError('x should be int')
    
    
    past_timedelta = datetime.timedelta(seconds=x)
    past = now - past_timedelta
    
    return past if not iso_format else past.isoformat()
